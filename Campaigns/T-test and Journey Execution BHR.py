...
JOIN gold.customer.vynamic_customer_profile AS t2
    ON t1.customer_key = t2.customer_key
    AND t1.card_key = t2.card_key
WHERE
    t2.LHRDate_utc IS NULL
    OR t2.loyalty_program_id IS NULL
"""
non_lhp_customers_flag = spark.sql(query).toPandas().empty

# If the data is empty, then display that all customers are loyalty
if non_lhp_customers_flag:
...
"""
non_bahrain_customers_flag = spark.sql(query).toPandas().empty

# If the data is empty, then display that all customers are loyalty
if non_bahrain_customers_flag:
    print('All customers are bahrain customers')

# Otherwise raise error
else:
    raise ValueError("Data consists of non-bahrain customers!")
...
significance_level = 0.05 # 0.05 = 5%
iterations = 1000
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS dev.analytics.bahrain_campaign_customer_details (
# MAGIC     campaign_id INT,
# MAGIC     campaign_name STRING,
# MAGIC     campaign_set STRING,
...
query = f"""
SELECT *
FROM dev.analytics.bahrain_campaign_customer_details
WHERE
...
if data_empty:
    ...
    spark_df.write.option("overwriteSchema", "true").mode("append").saveAsTable("dev.analytics.bahrain_campaign_customer_details")

    spark_df = spark.createDataFrame(test_cardkeys_df)
    spark_df.write.option("overwriteSchema", "true").mode("append").saveAsTable("dev.analytics.bahrain_campaign_test_cardkeys")

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT campaign_id, campaign_name, COUNT(*)
# MAGIC FROM dev.analytics.bahrain_campaign_customer_details
# MAGIC GROUP BY 1, 2
...
# MAGIC %sql
# MAGIC SELECT campaign_id, campaign_name, COUNT(*)
# MAGIC FROM dev.analytics.bahrain_campaign_test_cardkeys
# MAGIC GROUP BY 1, 2
# MAGIC ORDER BY 1 DESC
